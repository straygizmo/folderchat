...
{
    public partial class OllamaProviderSettingsControl : UserControl, IProviderSettingsControl
    {
        public OllamaProviderSettingsControl()
        {
            InitializeComponent();
        }
        ...
        {
            txtApiUrl.Text = string.IsNullOrEmpty(Properties.Settings.Default.Ollama_ApiUrl)
                ? "http://localhost:11434"
                : Properties.Settings.Default.Ollama_ApiUrl;

            // Load model list
            LoadModelList();
            chkSupportSystemMessage.Checked = Properties.Settings.Default.Ollama_SupportsSystemRole;
        }

        public void SaveSettings()
        {
            Properties.Settings.Default.Ollama_ApiUrl = txtApiUrl.Text;

            // Save selected model if any
            if (cmbModelList.SelectedItem != null)
            {
                Properties.Settings.Default.Ollama_SelectedModel = cmbModelList.SelectedItem.ToString();
            }

            // Save model list
            SaveModelList();
            Properties.Settings.Default.Ollama_SupportsSystemRole = chkSupportSystemMessage.Checked;
        }
        ...
        private void LoadModelList()
        {
            ...
            if (!string.IsNullOrEmpty(Properties.Settings.Default.Ollama_ModelList))
            {
                var models = Properties.Settings.Default.Ollama_ModelList.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
            ...
            if (!string.IsNullOrEmpty(Properties.Settings.Default.Ollama_SelectedModel) &&
                cmbModelList.Items.Contains(Properties.Settings.Default.Ollama_SelectedModel))
            {
                cmbModelList.SelectedItem = Properties.Settings.Default.Ollama_SelectedModel;
            }
        ...
        private void SaveModelList()
        {
            ...
            Properties.Settings.Default.Ollama_ModelList = string.Join(",", models);
        }
        ...
        private async void BtnFetch_Click(object? sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var response = await httpClient.GetAsync($"{apiUrl}/api/tags");
                ...
                var modelsResponse = System.Text.Json.JsonSerializer.Deserialize<OllamaModelsResponse>(json);
                ...
                if (modelsResponse?.models != null && modelsResponse.models.Count > 0)
                {
                    ...
                    {
                        cmbModelList.Items.Add(model.name);
                    }
