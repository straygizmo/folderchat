...
{
    public partial class OpenAIAPIProviderSettingsControl : UserControl, IProviderSettingsControl
    {
        public OpenAIAPIProviderSettingsControl()
        {
            InitializeComponent();
        }
        ...
        {
            txtApiUrl.Text = string.IsNullOrEmpty(Properties.Settings.Default.OpenAIAPI_ApiUrl)
                ? "https://api.openai.com/v1"
                : Properties.Settings.Default.OpenAIAPI_ApiUrl;
            txtApiKey.Text = Properties.Settings.Default.OpenAIAPI_ApiKey;

            // Load model list
            LoadModelList();
            chkSupportSystemMessage.Checked = Properties.Settings.Default.OpenAIAPI_SupportsSystemRole;
        }

        public void SaveSettings()
        {
            Properties.Settings.Default.OpenAIAPI_ApiUrl = txtApiUrl.Text;
            Properties.Settings.Default.OpenAIAPI_ApiKey = txtApiKey.Text;

            // Save selected model if any
            if (cmbModelList.SelectedItem != null)
            {
                Properties.Settings.Default.OpenAIAPI_SelectedModel = cmbModelList.SelectedItem.ToString();
            }

            // Save model list
            SaveModelList();
            Properties.Settings.Default.OpenAIAPI_SupportsSystemRole = chkSupportSystemMessage.Checked;
        }
        ...
        private void LoadModelList()
        {
            ...
            if (!string.IsNullOrEmpty(Properties.Settings.Default.OpenAIAPI_ModelList))
            {
                var models = Properties.Settings.Default.OpenAIAPI_ModelList.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
            ...
            if (!string.IsNullOrEmpty(Properties.Settings.Default.OpenAIAPI_SelectedModel) &&
                cmbModelList.Items.Contains(Properties.Settings.Default.OpenAIAPI_SelectedModel))
            {
                cmbModelList.SelectedItem = Properties.Settings.Default.OpenAIAPI_SelectedModel;
            }
        ...
        private void SaveModelList()
        {
            ...
            Properties.Settings.Default.OpenAIAPI_ModelList = string.Join(",", models);
        }
